...
public class AccountController implements IAccountController {
    ...
    public Object getAccountById(@PathVariable Long id, Authentication authentication) {
        List<GrantedAuthority> actualRoles = new ArrayList<>(authentication.getAuthorities());
        for (GrantedAuthority auth : actualRoles) {
            System.out.println(auth.getAuthority());
        }
        Optional<Account> result = accountRepository.findAccountById(id);

        if (result.isPresent()) {
            if (actualRoles.stream().anyMatch(a -> a.getAuthority().equalsIgnoreCase("ROLE_ADMIN"))) {
                return result.get();
            } else {
    ...
    public List<UserAccountBalanceDTO> getAccountsBalance(Authentication authentication) {
        User actualUser = (User) authentication.getPrincipal();
        //String actualUsername = customUserDetails.getUsername();

        return accountService.getAccountsWithBalance(actualUser.getUsername());
    }

    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @GetMapping(value = "/account/{id}/basic-details")
    public UserAccountBalanceDTO getAccountBasicDetails(@PathVariable Long id) {
    ...
    public UserAccountBalanceDTO updateAccountBalance(@PathVariable Long id,@RequestParam(name = "amount") Double amountInput) {
        if (amountInput <= 0) {
    ...
    @ResponseStatus(HttpStatus.OK)
    public UserAccountBalanceDTO updateAccountBalance(@PathVariable Long id, @RequestParam(name = "status") String statusInput) {
        try {
